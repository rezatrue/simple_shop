...
if (isset($_GET['o_id'])){
    $result = $db->partialClosedOrderIdListPage($like_o_id, $page, $itemsPerPage);
    $totalItems = $db->countForPartialClosedOrderIdListPage($like_o_id);
} else{
    $result = $db->closedOrderListPage($page, $itemsPerPage);
    $totalItems = $db->countForClosedOrderListPage();
}
...
<!-- <div class="table-responsive mb-2">
    <tbody>
        <tr> -->
            <form action="closed_orders.php" method="get" id="searchForm">
            <div class="row">
...
<div class="card card-primary">  
  <div class="card-header">
    <h3 class="card-title">Closed Orders (total: <?php echo $totalItems; ?>)</h3>
  </div>
  <!-- /.card-header -->
    ...
    <div class="table-responsive"> 
       <table class="table table-striped table-hover w-auto">
          <thead class="table-light">
            <tr>
            <th><h5>Dalivery_Date</h5></th>
            <th><h5>Order_Info</h5></th>
            <th><h5>Items</h5></th>
            <th><h5>Address</h5></th>
            <th><h5>Notes</h5></th>
            </tr>
                    ...
                    <?php 
                        if (!empty($result['order'])) {
                            foreach ($result['order'] as $row) {
                                include('./pages/admin/closed_order_list_single_row.php');
                            }
                        } else {
                            echo "0 results";
                        }
